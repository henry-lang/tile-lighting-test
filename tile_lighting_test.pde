...
final int LIGHT_RADIUS = 8;
final float LIGHT_CUTOFF = 0.02;
final float NORMAL_DROPOFF = 0.7;
...
void setup() {
  ...
  lightTexture = createImage(lighting.length, lighting.length, RGB);
  
}

void draw() {
  background(10, 250, 255);
...
/*void mouseMoved() {
  if((int) (mouseX / 20) != (int) (pmouseX / 20) || (int) (mouseY / 20) != (int) (pmouseY / 20)) {
    tiles[(int) (mouseX / 20)][(int) (mouseY / 20)] = tileTypes.RED_LIGHT;
    tiles[(int) (pmouseX / 20)][(int) (pmouseY / 20)] = tileTypes.STONE;
    calculateLighting();
  }
}*/
...
void drawTiles() {
  ...
  for (var x = 0; x < tiles.length; x++) {
    for (var y = 0; y < tiles[x].length; y++) {
      image(tileTypes.textures[tiles[x][y]], x * 20, y * 20, 20, 20);
    }
  }
}
...
void calculateLighting() {
  ...
  for (var x = 0; x < lighting.length; x++) {
    for (var y = 0; y < lighting[x].length; y++) {
      lightTexture.pixels[x + y * 40] = lighting[x][y];
    }
  }
